...
class Cryspy:
    ...
    def createSetup(self, key: str = 'setup', cls_type: Optional[str] = None):
        ...
        if cls_type == 'powder1DCW':
            setup = cryspy.Setup(wavelength=self.conditions['wavelength'], offset_ttheta=0, field=0)
    ...
    def powder_1d_calculate(self, x_array: np.ndarray, full_return: bool = False, **kwargs):
        """
    ...
    def powder_1d_tof_calculate(self, x_array: np.ndarray, pol_fn=None, full_return: bool = False, **kwargs):
        """
        For a given x calculate the corresponding y
        :param x_array: array of data points to be calculated
        :type x_array: np.ndarray
    ...
    def do_calc_setup(self, scale: float, this_x_array: np.ndarray, pol_fn: Callable) -> Tuple[np.ndarray, dict]:
    ...
    def _do_run(self, model, polarized, x_array, crystals, phase_list, bg, phase_scales):
        ...
        if is_tof:
            ...
            self._cryspyDict[exp_name_model]['flags_background_intensity'] = np.full(
                len(self.pattern.backgrounds[0].x_sorted_points), False
            )
